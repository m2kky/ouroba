    ...
    <div className="bg-gray-50 min-h-screen py-16">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-8">
          ...
          <span>/</span>
          {/* We would ideally show brand name here but our product endpoint might need to include it, or we rely on the user knowing. Let's just say "Products" */}
        ...
        <div className="bg-white rounded-3xl p-8 md:p-12 shadow-sm border border-gray-100 flex flex-col md:flex-row gap-12">
          ...
          <div className="w-full md:w-1/2">
            <div className="aspect-square bg-gray-50 rounded-2xl flex items-center justify-center p-8 relative">
          ...
          <div className="w-full md:w-1/2 flex flex-col">
            ...
            <h1 className="text-4xl font-bold text-gray-900 mb-4">{product.nameEn}</h1>
            
            <div className="text-xl font-medium text-gray-500 mb-8">
              Weight / Volume: <span className="text-gray-900">{product.weight}</span>
            </div>

            <div className="prose prose-green max-w-none text-gray-600 mb-8">
              <h3 className="text-lg font-bold text-gray-900 mb-2">Description</h3>
              <p className="leading-relaxed">{product.descriptionEn || "No description available."}</p>
            </div>

            {/* Recipes that use this product */}
            {product.recipes?.length > 0 && (
              <div className="mt-auto border-t border-gray-100 pt-8">
                <h3 className="text-lg font-bold text-gray-900 mb-4">Made with this product</h3>
                <div className="flex gap-4">
                  {product.recipes.map((rp: any) => (
                    <Link 
                      key={rp.recipe.id} 
                      href={`/recipes/${rp.recipe.id}`}
                    ...
                    >
                      ...
                      <span className="font-semibold text-sm text-gray-700">{rp.recipe.nameEn}</span>
                    </Link>
                  ))}
                </div>
              </div>
