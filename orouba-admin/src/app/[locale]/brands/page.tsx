...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: locale === 'ar' ? "منتجاتنا | Orouba Foods" : "Our Brands | Orouba Foods",
    description: locale === 'ar' 
      ? "استكشف علاماتنا التجارية الرائدة ومجموعة منتجاتنا الواسعة."
      : "Explore our leading brands and wide range of products.",
  };
}
    ...
    <div className="bg-gray-50 min-h-screen py-16 pt-32">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-5xl font-bold text-orouba-blue mb-6">
          ...
          <p className="mt-6 text-gray-600 max-w-2xl mx-auto text-lg leading-relaxed">
            {locale === 'ar' 
              ...
              : 'Discover our trusted brands, each dedicated to providing specific and high-quality food product lines.'}
          </p>
        </div>
            ...
            >
              <div className="h-48 w-48 mx-auto mb-8 relative overflow-hidden rounded-full flex items-center justify-center p-6">
                {brand.image ? (
                  <img
                    src={brand.image}
                    alt={t(locale, brand.nameAr, brand.nameEn)}
                    className="object-contain w-full h-full group-hover:scale-110 transition-transform duration-500"
                  />
                ) : (
                  <span className="text-5xl text-orouba-blue font-bold">{t(locale, brand.nameAr, brand.nameEn)}</span>
                )}
              </div>
              <h2 className="text-3xl font-bold text-center mb-4 text-orouba-blue">
                {t(locale, brand.nameAr, brand.nameEn)}
              </h2>
              <p className="text-gray-600 mb-8 text-center flex-grow leading-relaxed">
                {t(locale, brand.descriptionAr, brand.descriptionEn)}
              </p>
              
              <div className="mt-auto">
                <div className="flex justify-between items-center pt-6 border-t border-gray-100 mb-6 text-sm text-gray-500 font-medium">
                  <span>{brand.categories?.length || 0} {locale === 'ar' ? 'فئات' : 'Categories'}</span>
                  <span>
                    {brand.categories?.reduce((acc: number, cat: any) => acc + (cat.products?.length || 0), 0)} {locale === 'ar' ? 'منتجات' : 'Products'}
                  </span>
                </div>
                <Link
