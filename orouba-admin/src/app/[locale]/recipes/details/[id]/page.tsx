...
export default async function RecipeDetailsPage({
  ...
  if (!recipe || recipe.isHidden) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center bg-gray-50 pt-20">
        <h1 className="text-3xl font-bold text-orouba-blue mb-4">{locale === "ar" ? "الوصفة غير موجودة" : "Recipe Not Found"}</h1>
        <Link href={`/${locale}/recipes`} className="text-orouba-yellow bg-orouba-blue px-6 py-2 rounded-full hover:bg-blue-800 transition-colors">
          {locale === "ar" ? "العودة للوصفات" : "Back to Recipes"}
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white min-h-screen pb-10">
      ...
      {/* 1. Recipe Banner (Image and Title) */}
