...
export default function SettingsPage() {
  ...
  const handleRemove = (index: number) => {
    if (confirm("هل أنت متأكد من إزالة هذا الإعداد؟ سيتم حذفه عند الحفظ.")) {
  ...
  const defaultKeys = [
    { key: "site_name", desc: "اسم الموقع" },
    { key: "phone_1", desc: "رقم الهاتف الأساسي" },
    { key: "phone_2", desc: "رقم الهاتف البديل" },
    { key: "email_support", desc: "البريد الإلكتروني للدعم" },
    { key: "address", desc: "العنوان الرئيسي" },
    { key: "facebook_url", desc: "رابط فيسبوك" },
    { key: "instagram_url", desc: "رابط إنستجرام" },
    { key: "linkedin_url", desc: "رابط لينكد إن" },
    { key: "whatsapp_number", desc: "رقم واتساب" },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">إعدادات الموقع (Settings)</h1>
          <p className="text-gray-500 mt-1">تعديل المتغيرات الديناميكية للموقع (الهاتف، الإيميل، العناوين، الروابط)</p>
        </div>
        <button
          onClick={handleSave}
          disabled={isSaving}
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden" dir="rtl">
          <div className="p-4 border-b border-gray-100 bg-gray-50 flex gap-2 overflow-x-auto hide-scrollbar">
            <span className="text-sm font-semibold text-gray-600 flex items-center shrink-0">إضافة مفاتيح شائعة:</span>
            {defaultKeys.map(dk => (
              <button 
                key={dk.key}
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-12 gap-4 font-bold text-sm text-gray-500 border-b pb-2 px-2">
              <div className="col-span-3">المفتاح (Key)</div>
              <div className="col-span-4">القيمة (عربي)</div>
              <div className="col-span-4">القيمة (إنجليزي)</div>
